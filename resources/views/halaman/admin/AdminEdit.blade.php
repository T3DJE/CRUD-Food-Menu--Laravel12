    ...
    <div class="section">
            ...
            <div class="mBox">
                ...
                <div class="box">
                    ...
                    @foreach ($data2 as $items)
                        <img src="{{ asset('images/' . $items->image) }}" width="150" class="imgg"/>
                    @endforeach
                    <br>
                    <div class="btnAction">
                        <a href="{{ $item->id }}"></a>
                        <a href="/create">Batal</a>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="input-method">
            <div class="link-add-purchase">
                <a href="/create">Edit Product</a>
            </div>
            <div class="form-box">
                <form action="/process-edit" method="post" enctype="multipart/form-data" class="formd">
                    @csrf
                    @foreach ($data as $item)
                    <input name="id" type="hidden" value="{{ $item->id }}"/>
                    <label for="nama_product" class="pemail">Product Name</label>
                    <input name="nama_product" type="text" placeholder="Product Name" id="nama_product" value="{{ $item->nama_product }}"/>
                    <label for="harga" class="pemail">Price</label>
                    <input name="harga" type="number" placeholder="Price" min="0" id="harga" value="{{ $item->harga }}"/>
                    <label for="stok" class="pemail">Stock</label>
                    <input name="stok" type="number" placeholder="Stock" min="0" id="stok" value="{{ $item->stok }}"/>
                    <label for="kategori" class="pemail">Category</label>
                    <select name="kategori" id="kategori" value="{{ $item->kategori }}">
                    ...
                    <input name="description" type="text" placeholder="Description" id="description" value="{{ $item->description }}"/>
                    <div class="btnImage">
                        <label for="image" class="bi">Image</label>
                        <label for="image" class="edit-file-button">Choose File</label>
                        <input type="file" name="image" id="image"/>
                    </div>
                    <button class="btnAdd">Edit Product</button>
                    @endforeach
                </form>
            </div>
        </div>
